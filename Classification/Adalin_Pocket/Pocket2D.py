...
def Pocket2D(x, y, t_max = 100):
    ...
    for k in range(len(x)):
        if np.sign(np.dot(np.transpose(ws),np.array([x[k][0],x[k][1], 1]))) != y[k] :
            ws = ws + y[k] * (np.array([x[k][0], x[k][1], 1])) 
           
    lossfuncWs = lossfunction(ws, x, y)

    wt = [0, 0, 1]
    for i in range(0, t_max):
        for j in range(len(x)):
            if np.sign(np.dot(np.transpose(wt), np.array([x[j][0],x[j][1], 1]))) != y[j] :
                wt = wt + y[j] * (np.array([x[j][0], x[j][1], 1])) 
                compteur+=1
                #----evaluation de Ls(w(t)) 
                lsWt = lossfunction(wt, x, y)
                if(lsWt < lossfuncWs):
                    ws = wt
                    lossfuncWs = lossfunction(ws, x, y)
                        
                #-------Plot de separateur----------------------------------------#
                if compteur % 1000 == 0:#pour minimise le nbr des plot 1 seul plot pour chaque 1000 iteration
                    ...
                    if ( ws[1]!= 0 ):
                        k = (-ws[0]/ws[1])* z - ws[2]/ws[1]
                        plt.plot(z , k, color='green')
            
                    plt.pause(0.05)
                    plt.show()
         
    erreur_dapro = lossfunction(ws, x, y)
    return ws, erreur_dapro,  compteur


#-----la hase d'entrenemment de model ----------------------------------------#
theta , erreur_daproximation, compteur = Pocket2D(X_train, y_train)
print("Erreur d'aproximation = ", erreur_daproximation, "\nle nomber d'itiration = ", compteur)
